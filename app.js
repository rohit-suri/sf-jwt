...
var token_url = new url.URL(
  '/services/oauth2/token',
  credentials.salesforce.url,
).toString()

axios.post(token_url, querystring.stringify(params)).then(function (res) {
  var conn = new jsforce.Connection({
    instanceUrl: res.data.instance_url,
    accessToken: res.data.access_token,
  })
  console.log(`access token: ${conn.accessToken}`)
  conn.query('select CaseNumber, Subject from Case limit 1', function (
    err,
    results,
  ) {
